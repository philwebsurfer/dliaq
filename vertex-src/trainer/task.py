...
def train_model(model, train_data,  validation_data,
                ...
                compile_model=True):
  logging.info("Compilation flag: %s"%compile_model)
  if compile_model:
    logging.info('Compiling model...')
    model.compile(loss=loss, optimizer=optimizer, metrics=metrics)
  else:
    logging.info('Model was not compiled...')
...
def execute_train(window_size_days=2, sequence_length=None,
        stride=1, sampling_rate=1, 
        batch_size=128, steps=100, epochs=10, model_file="",
        input_dataset="", output_datastore="", compile_model=True):
  # Data Prep
  ...
  logging.info(f"```train2.shape={train2.shape}```")
  logging.info(f"```X_train.shape={X_train.shape}```")
  logging.info(f"```Y_train.shape={Y_train.shape}```")
  logging.info(f"```test.shape={test.shape}```")
  logging.info(f"```test2.shape={test2.shape}```")
  logging.info(f"```X_test.shape={X_test.shape}```")
  logging.info(f"```Y_test.shape={Y_test.shape}```")
  logging.info(f"timeseries_dataset_from_array(\
          sequence_length={past},\
          sampling_rate={sampling_rate},\
          stride={stride},\
          batch_size={batch_size},\
  ...
  trained_model = train_model(model, train_data=train3_iaq,
...
def usage(argv):
    ...
    history_grp = parser.add_mutually_exclusive_group()
    history_grp.add_argument('--window_size_days', '-w', nargs='?', default=2, type=int,
            help="""Window Size in Days. Default: 2.""")
    history_grp.add_argument('--sequence_length', '-l', nargs='?', default=None, type=int,
            help="""Sequence length of window history size. Default: None.""")
    ...
    parser.add_argument('--nocompile', '-c', action="store_true",  default=False,
                       help='Keeps hyperparameters of saved Tensorflow Model by\n'+
                            'not recompiling model.')
...
def main(argv):
    ### read cli arguments
    args = usage(argv)
    print("No compile flag: %s"%args.nocompile)
    print("Compile: %s"%(not args.nocompile))
    print("Window size: %s"%args.window_size_days)
    print("Sequence length: %s"%args.sequence_length)
    print("")
    #print(args)
